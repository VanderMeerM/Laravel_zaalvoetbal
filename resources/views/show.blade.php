...
<div class="flex items-center justify-center mt-2 bg-black p-7 w-80 m-auto">
 ...
 type="text" style="font-size: 50px;" id="goals_orange" name="goals_orange" 
 @if (Auth::user()->isAdmin !== 'on') disabled @endif
 value= {{ $current_date_id-> result_orange }}
 >
 ...
 <input 
class="block w-24 rounded-md bg-yellow-300 py-7 text-center text-gray-900 outline -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6"
type="text" style="font-size: 50px;"  id="goals_yellow" name="goals_yellow" 
 @if (Auth::user()->isAdmin !== 'on') disabled @endif 
 value= {{ $current_date_id-> result_yellow }} 
>
